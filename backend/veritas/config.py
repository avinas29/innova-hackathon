...
class Settings(BaseSettings):
    ...
    # Flash for the strong role: the better adjudicator, at 20 requests/DAY.
    #
    # That is a deliberate quality-over-volume trade. ~8 strong calls per run
    # means roughly 2 runs a day, which is enough for a demo and gives sharper
    # verdicts than Flash-Lite. Set this to gemini-3.1-flash-lite for 1,000/day
    # (~22 runs) when you need volume instead.
    #
    # Safe to run this close to the edge only because exhaustion is now handled:
    # the daily count is tracked with the correct limit, and a per-day 429 fails
    # immediately with an actionable message rather than being retried for
    # minutes.
